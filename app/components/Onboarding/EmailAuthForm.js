...
export default function EmailAuthForm({ onSubmit, isLoading = false, variant = "full" }) {
  ...
  const legalLinks = (
    <div className="text-center mt-3 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-xs text-brand-text-weak" style={{ fontFamily: "Open Sans, sans-serif" }}>
      <Link
  ...
  if (variant === "modal") {
    return (
      <div className="w-full flex flex-col items-center text-center">
        ...
        <h2
          id="guest-auth-title"
          className="text-brand-text-strong font-semibold mb-2 leading-tight"
          style={{ fontFamily: "var(--font-geist-sans), Open Sans, sans-serif", fontSize: "28px" }}
        >
          Sign in to apply
        </h2>
        <p className="text-sm text-brand-text-weak mb-6" style={{ fontFamily: "Open Sans, sans-serif" }}>
          Create an account or log in to continue.
        </p>
        {oauthButtons}
        <div className="mt-1 pb-1">{legalLinks}</div>
      </div>
    );
  }
