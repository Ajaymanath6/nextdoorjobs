  ...
  const renderCompanyMarkers = (companies) => {
    ...
    companiesToRender.forEach((company, index) => {
      ...
      const customIcon = logoUrl
        ? createGeminiJobIcon(L, 50, logoUrl, positionsOpen, nameForBadge)
        : createOrgIconMarker(L, 50, positionsOpen, nameForBadge);
...
// Optional companyName + jobCount show two badges: company name (top, white bg), openings count (bottom, primary).
  ...
  const createGeminiJobIcon = (L, size = 50, logoUrl = null, jobCount = 0, companyName = null) => {
    ...
    const hasNameBadge = companyName && String(companyName).trim();
    const hasCountBadge = jobCount > 0;
    const badgeCount = (hasNameBadge ? 1 : 0) + (hasCountBadge ? 1 : 0);
  ...
  const createOrgIconMarker = (L, size = 50, jobCount = 0, companyName = null) => {
    const badgeHeight = 18;
    const hasNameBadge = companyName && String(companyName).trim();
    const hasCountBadge = jobCount > 0;
    const badgeCount = (hasNameBadge ? 1 : 0) + (hasCountBadge ? 1 : 0);
      ...
      if (companies && companies.length > 0) {
        ...
        companiesToRender.forEach((company, index) => {
          ...
          const customIcon = companyLogoUrl
            ? createGeminiJobIcon(L, 50, companyLogoUrl, positionsOpen, nameForBadge)
            : createOrgIconMarker(L, 50, positionsOpen, nameForBadge);
