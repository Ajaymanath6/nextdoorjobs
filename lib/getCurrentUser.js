 ...
 * Returns { id, email, name, phone, accountType } or null if not authenticated.
...
export async function getCurrentUser() {
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
          try {
            return await prisma.user.findUnique({
              ...
              select: { id: true, email: true, name: true, phone: true, accountType: true },
            });
          } catch (e) {
            ...
            if (isTimeoutError(e) && i < MAX_RETRIES - 1) {
              await new Promise((r) => setTimeout(r, RETRY_DELAY_MS));
              continue;
            }
            console.warn("[getCurrentUser] Clerk email lookup failed:", e?.code || e?.message);
            return null;
          }
        }
        return null;
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    try {
      ...
      console.warn("[getCurrentUser] Cookie session lookup failed:", e?.code || e?.message);
      return null;
    }
  }
  return null;
