...
/** Canonical stored + legacy values that should match a work-mode filter. */
export function remoteTypeAliases(remoteTypeOrLabel) {
  if (remoteTypeOrLabel == null || remoteTypeOrLabel === "") return [];
  const raw = String(remoteTypeOrLabel).trim();
  const apiValue = workModeToApi(raw);
  const normalized = apiValue.toLowerCase().replace(/[_]/g, " ").trim();

  if (
    normalized === "on-site" ||
    normalized === "onsite" ||
    normalized === "work from office" ||
    normalized === "wfo" ||
    normalized === "office"
  ) {
    return ["On-site", "Work from office", "Onsite", "WFO", "Office"];
  }
...
/** Prisma `OR` clause for JobPosition.remoteType, or null if no filter. */
export function remoteTypePrismaOr(remoteTypeOrLabel) {
  const aliases = remoteTypeAliases(remoteTypeOrLabel);
  if (!aliases.length) return null;
  return {
    OR: aliases.map((value) => ({
      remoteType: { equals: value, mode: "insensitive" },
    })),
  };
}

/** Client-side work-mode match (same aliases as API). */
export function jobMatchesWorkMode(job, workModeLabel) {
  if (!workModeLabel) return true;
  const aliases = remoteTypeAliases(workModeLabel).map((a) => a.toLowerCase());
  const value = String(job?.remoteType || "")
    .trim()
    .toLowerCase();
...
/** Company jobs list — same job filters as map/companies feed. */
export function buildCompanyJobsQuery(companyId, filters = {}) {
  const params = new URLSearchParams();
  appendJobFilterParams(params, filters);
  const qs = params.toString();
  return qs
    ? `/api/companies/${companyId}/jobs?${qs}`
